...
from utils.remote_forms import logger, widgets


class RemoteField(object):
    """
    A base object for being able to return a Django Form Field as a Python dictionary.
    ...
    def as_dict(self):
        ...
        field_dict['class'] = self.field.__class__.__name__
...
class RemoteTimeField(RemoteField):
    def as_dict(self):
        ...
        field_dict['input_format'] = self.field.input_formats[0]

        if field_dict.get('initial'):
            ...
            if isinstance(field_dict['initial'], (datetime.date, datetime.datetime, datetime.time)):
                if type(field_dict['initial']).__name__ == 'date':
                    field_dict['input_format'] = settings.DATE_FORMAT
                elif type(field_dict['initial']).__name__ == 'datetime':
                    field_dict['input_format'] = settings.DATETIME_FORMAT
                elif type(field_dict['initial']).__name__ == 'time':
                    field_dict['input_format'] = settings.TIME_FORMAT
                field_dict['initial'] = field_dict['initial'].strftime(field_dict['input_format'])

        field_dict.pop('input_format', None)
        return field_dict
...
class RemoteRegexField(RemoteCharField):
    def as_dict(self):
        ...
        # We don't need the pattern object in the frontend field_dict['regex'] = self.field.regex
        return field_dict


class RemotePhoneNumberField(RemoteCharField):
    def as_dict(self):
        return super(RemotePhoneNumberField, self).as_dict()
...
class RemoteChoiceField(RemoteField):
    def as_dict(self):
        return super(RemoteChoiceField, self).as_dict()
