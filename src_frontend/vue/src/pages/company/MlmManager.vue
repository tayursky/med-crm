<template>
  <div>
    ...
    <div v-else class="form-block mlm-agent">
      ...
      <div v-if="person" class="mlm-agent__person">
        <div class="mlm-agent__person__title">{{ person.full_name }}</div>
      ...
      <div v-if="agent" class="mlm-agent__agent">
        <div class="mlm-agent__person__title"><strong>{{ agent.position__label }}</strong></div>
      ...
      <div style="padding: 0 0 5px; display: inline-block; width: 100%;">
        <div class="float-right">
          <el-button icon="el-icon-document-add" title="Создать нового агента" @click="agentAdd"/>
        </div>
      </div>
      ...
      <el-tabs v-model="activeTab" type="border-card">
        ...
        <el-tab-pane v-if="agent && agent.position === 'manager'" name="child_agents_total" label="Закрытые периоды">
          <el-row class="directory__head">
            <div v-if="!child_agents_total.loading" class="directory__total">
              Начислено как менеджеру: {{ agent.cache.manager_income }}
            </div>
            <div class="float-right">
        ...
        <el-tab-pane v-if="agent && agent.position === 'manager'" name="child_agents" label="Агенты">
          <el-row class="directory__head">
            <div v-if="!child_agents.loading" class="directory__total">
              Всего агентов: {{ child_agents.count }} |
        ...
        <el-tab-pane name="invites" label="Приглашения">
          <el-row class="directory__head">
            <div class="directory__total">Приглашений: {{ invites.count }} | Начислено: {{ invites.total }}</div>
        ...
        <el-tab-pane name="payments" label="Выплаты">
          <el-row class="directory__head">
            <div class="directory__total">Выплат: {{ payments.count }} | Выплачено: {{ payments.total }}</div>
            <div class="directory__buttons">
              <el-button icon="el-icon-refresh" title="Обновить" @click="paymentsLoad"/>
            </div>
          </el-row>
          <template v-if="!payments.loading">
            <data-table :headers="payments.headers" :items="payments.items" :loading="payments.loading"/>
          </template>
        </el-tab-pane>
    ...
    <the-dialog app_label="company" model_name="mlm_cabinet_manager" :item="agent_new" :permissions="permissions"/>
  ...
  export default {
    ...
    methods: {
      ...
      agentLoad() {
        ...
        this.loading = true;
        let params = {'get': true};
        let url = this.$store.getters.root_url + 'company/mlm_cabinet_manager/';
      ...
      invitesLoad() {
        ...
        let url = this.$store.getters.root_url + 'company/mlm_cabinet_manager/invite/';
        Vue.axios.get(url, {params: params})
          .then(response => {
            this.invites = {
      ...
      paymentsLoad() {
        ...
        let params = {'get': true, 'agent': this.agent_id};
        let url = this.$store.getters.root_url + 'company/mlm_cabinet_manager/payments/';
        Vue.axios.get(url, {params: params})
          .then(response => {
            this.payments = {
              'filters': response.data.filters,
      ...
      agentAdd() {
        this.agent_new = {'id': 'add'};
      },

      formSave(action) {
        this.loading = true;
        let url = this.$store.getters.root_url + 'company/mlm_cabinet_manager/' + this.agent_id + '/' + action + '/';
        Vue.axios.post(url, this.getFormData(this.data))
          .then(response => {
            if (response.data.form.errors) {
