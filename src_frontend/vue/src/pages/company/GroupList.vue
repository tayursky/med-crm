  ...
  export default {
    ...
    methods: {
      ...
      getAjaxParams: function (from = null) {
        ...
        if (this.filters) {
          for (let key in this.filters.data) {
            params[key] = this.list_filters.data[key];
          }
        }
        return params;
      },
      ...
      loadModel() {
        if (!this.loading) {
          this.loading = true;
          Vue.axios.get(this.$store.getters.root_url + 'company/group/', {params: this.getAjaxParams()})
            .then(response => {
      ...
      editItem(item) {
        ...
        this.$router.push({name: 'company_group', params: {'group_id': item.id}})
      }
